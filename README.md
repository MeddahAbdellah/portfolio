...
Chat answers are streamed from the OpenAI Responses API to the browser as newline-delimited JSON, so text appears as it is generated. Because `api/chat.js` is a plain Vercel `/api` function, its 300-second duration is configured at the platform level in `vercel.json`; keeping this setting outside the function avoids framework-specific route-export handling. The function aborts an upstream model response after 240 seconds. Individual GitHub requests allow 30 seconds, reducing failures caused by transient API latency.
