...
Chat answers are streamed from the OpenAI Responses API to the browser as newline-delimited JSON, so text appears as it is generated. Because `api/chat.js` is a plain Vercel `/api` function, its duration is configured at the platform level in `vercel.json`; keeping this setting outside the function avoids framework-specific route-export handling. The function duration is 60 seconds for compatibility with projects that do not have Vercel Fluid compute enabled, and the upstream request aborts after 55 seconds so the browser receives a structured timeout instead of an abruptly truncated stream. Projects with Fluid compute enabled can raise both values within their plan limits. Individual GitHub requests allow 30 seconds, reducing failures caused by transient API latency.
