...
Chat answers are streamed from the OpenAI Responses API to the browser as newline-delimited JSON, so text appears as it is generated. The chat route exports `maxDuration = 280`, allowing Vercel to run it for up to 280 seconds. The application does not impose a shorter upstream response timeout; it only aborts the OpenAI request if the browser connection closes. Individual GitHub requests allow 30 seconds, reducing failures caused by transient API latency.
